...
- 多周期小时观察 `mtf-long-notify`: 4.5 小时内必须成功运行一次；无新增开仓/平仓时不要求发送 Telegram
- 周复盘 `weekly-all`: 8 天内必须成功一次

健康检查每小时运行一次。发现异常时会通过 Telegram 发告警；同一异常 6 小时内不会重复刷屏。

多周期小时观察只承担逐笔开仓/平仓提醒。没有新增交易时，它只写本地日志，不推送重复复盘。健康检查会识别“任务正在运行”的状态，避免和小时观察同时启动时误报。每日复盘和周复盘仍必须完成 Telegram 推送。
